...
<body>
 <header>
   ...
   <main class="mt-5 pt-5">
        ...
        <?php
        if(isset($_GET['IdClienteB'])){
            $client = new SoapClient("http://100.26.227.87:8080/Autobus1.wsdl");
        
            $parametros = array("IdCliente"=> $_GET['IdClienteB']);
            $response = $client->__soapCall('ConsultarCliente',array($parametros),);
            print"<h2>".$response->{'NombreCliente'}.' '.$response->{'Apellidos'}. ' '.$response->{'IdCliente'}."</h2>";
        }
    ?>
...
<a href="ConsultarCliente.php" class="login-forgot-pass1">Viajes Lupis</a>
